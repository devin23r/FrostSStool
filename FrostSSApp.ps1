...
$script:Colors = @{
    Background = Get-UiColor '#060708'
    Surface    = Get-UiColor '#0B0D0F'
    Panel      = Get-UiColor '#121518'
    Border     = Get-UiColor '#252A2F'
...
function Scan-ModFiles {
    ...
    foreach ($directory in $directories) {
        ...
        foreach ($file in $files) {
            ...
            $metadataMatches = @(Get-IndicatorMatches -Text (Read-JarMetadata -Path $file.FullName) -Context Metadata)
...
function Verify-OneFile {
    ...
    $script:ScannedFiles++
    $matches = @(Get-IndicatorMatches -Text $file.Name -Context ArtifactName)
    if ($file.Extension -ieq '.jar') {
        $matches += @(Get-IndicatorMatches -Text (Read-JarMetadata -Path $file.FullName) -Context Metadata)
    }
...
function New-NavButton {
    ...
    $button.Width = 162
    $button.Height = 34
    $button.Margin = [System.Windows.Forms.Padding]::new(0, 0, 0, 5)
...
$titleMark.BackColor = $script:Colors.Text
...
$nav.Location = [System.Drawing.Point]::new(12, 70)
$nav.Size = [System.Drawing.Size]::new(166, 590)
