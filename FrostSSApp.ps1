...
function Scan-ModFiles {
    ...
    foreach ($directory in $directories) {
        ...
        foreach ($file in $files) {
            ...
            $matches = @($nameMatches + $metadataMatches + $structureMatches | Sort-Object Name -Unique)

            foreach ($match in $matches) {
                $source = if (@($nameMatches | Where-Object { $_.Name -eq $match.Name }).Count -gt 0) {
                    'Filename match'
            ...
            try {
                Get-ChildItem -LiteralPath $location -Force -ErrorAction SilentlyContinue |
                    Select-Object -First 2500 | ForEach-Object {
                        $script:ScannedFiles++
                        foreach ($match in @(Get-IndicatorMatches -Text $_.Name -Context ArtifactName)) {
    ...
    if (Test-Path -LiteralPath $prefetch -PathType Container) {
        try {
            foreach ($file in @(Get-ChildItem -LiteralPath $prefetch -File -Filter '*.pf' -ErrorAction Stop)) {
                foreach ($match in @(Get-IndicatorMatches -Text $file.Name -Context Process)) {
