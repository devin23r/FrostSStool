...
function Read-JarMetadata {
    ...
    try {
        ...
        foreach ($entry in $archive.Entries) {
            ...
            if ($entryCount -lt 25000 -and $entryText.Length -lt 3000000) {
            ...
            } elseif ($normalized.EndsWith('.class', [StringComparison]::OrdinalIgnoreCase) -and $entry.Length -le 1048576 -and $classBytesRead -lt 6291456) {
                $stream = $entry.Open()
                $memory = [System.IO.MemoryStream]::new()
                try {
                    $stream.CopyTo($memory)
                    $bytes = $memory.ToArray()
                    $remaining = 6291456 - $classBytesRead
            ...
            try {
                Get-ChildItem -LiteralPath $location -Force -ErrorAction SilentlyContinue |
                    Select-Object -First 2500 | ForEach-Object {
                        $script:ScannedFiles++
                        $artifactMatches = @(
                            @(Get-IndicatorMatches -Text $_.Name -Context ArtifactName) +
                            @(Get-CheatFeatureMatches -Text $_.Name -Context ArtifactName) |
